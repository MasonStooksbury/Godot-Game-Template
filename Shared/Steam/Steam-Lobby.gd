...
@onready var send_message_text_edit = $SendMessageButton/LineEdit
...
func sendChatMessage():
	...
	var message = send_message_text_edit.text
	if message == '':
		return
	# Clear chat message input text
	send_message_text_edit.clear()
...
func _loaded_avatar(player_steam_id: int, size: int, buffer: PackedByteArray):
	...
	Global.LOBBY_MEMBERS[getPlayerIndexBySteamID(str(player_steam_id))].profile_picture = AVATAR_TEXTURE
...
func _on_create_lobby_button_pressed():
	if create_lobby_text_edit.text == '':
		return
	createLobby()
